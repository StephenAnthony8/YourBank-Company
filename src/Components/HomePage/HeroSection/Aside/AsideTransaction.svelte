<script lang="ts">
  const transactionList: {
    action: string;
    userName: string;
    amount: string;
  }[] = [
    { action: "Transaction", userName: "John Doe", amount: "$500.00" },
    { action: "Transaction", userName: "Jane Smith", amount: "$200.00" },
    { action: "Transaction", userName: "Alice Johnson", amount: "$300.00" },
    { action: "Transaction", userName: "Bob Brown", amount: "$150.00" },
    /* { action: "Transaction", userName: "Charlie White", amount: "$400.00" },
    { action: "Transaction", userName: "Diana Green", amount: "$250.00" },
    { action: "Transaction", userName: "Ethan Blue", amount: "$600.00" },
    { action: "Transaction", userName: "Fiona Black", amount: "$350.00" }, */
  ];
</script>

<div class="hero-section-transactions flex">
  <h4 class="capitalize">Your Transactions</h4>
  <div class="carousel-container flex">
  <ul class="hero-section-transactions-list carousel">
    {#each transactionList as { action, userName, amount }}
      <li class="flex carousel-options" id={userName}>
        <!-- Transaction icon(s) -->
        <div class="icon-container green-60 circle flex center">
          <svg
            viewBox="0 0 21 21"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            class="icon-sizing"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M13.8756 2.32065C13.5823 2.63646 13.6006 3.1302 13.9164 3.42345L16.1018 5.45273L7.16427 5.45273C6.73331 5.45273 6.38394 5.8021 6.38394 6.23307C6.38394 6.66403 6.73331 7.0134 7.16427 7.0134L16.1018 7.0134L13.9164 9.04268C13.6006 9.33593 13.5823 9.82967 13.8756 10.1455C14.1688 10.4613 14.6625 10.4796 14.9784 10.1863L18.6199 6.80489C18.7789 6.65724 18.8693 6.45005 18.8693 6.23306C18.8693 6.01608 18.7789 5.80889 18.6199 5.66124L14.9784 2.2798C14.6625 1.98655 14.1688 2.00484 13.8756 2.32065ZM7.21587 10.6442C6.92262 10.3284 6.42888 10.3101 6.11307 10.6033L2.47152 13.9848C2.31252 14.1324 2.22217 14.3396 2.22217 14.5566C2.22217 14.7736 2.31252 14.9808 2.47152 15.1284L6.11307 18.5099C6.42888 18.8031 6.92262 18.7848 7.21587 18.469C7.50913 18.1532 7.49084 17.6595 7.17503 17.3662L4.98965 15.3369H13.9272C14.3581 15.3369 14.7075 14.9876 14.7075 14.5566C14.7075 14.1256 14.3581 13.7763 13.9272 13.7763H4.98965L7.17503 11.747C7.49084 11.4537 7.50913 10.96 7.21587 10.6442Z"
              fill="#1C1C1C"
            />
          </svg>
        </div>
        <span class="transaction-context ">
          {action}
          <div class="transaction-container flex">

            <p class="transaction-context-label">{userName}</p>
            <p class="transaction-context-amount">- {amount}</p>
          </div>
        </span>
      </li>
    {/each}
  </ul>

  
  
  </div>
</div>

<style>
  .carousel-container {
    height: 240px;
    flex-direction: column;
    overflow: hidden;
  }
   .hero-section-transactions {
    --gap: 17px;
    flex-direction: column;
    flex: 1 0 0;
   }
   .hero-section-transactions-list {
    height: 180px;
   }
   .hero-section-transactions-list li {
    padding: 11px 17px;
   }
</style>
